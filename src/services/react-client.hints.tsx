...
const ClientHintsContext = React.createContext(null);

export const ClientHints = ClientHintsContext.Consumer;
interface ClientHintsProviderProps {
  header: { [key: string]: string };
  children: React.ReactNode;
}
export const ClientHintsProvider = ({ header = {}, children }: ClientHintsProviderProps) => {
  ...
  useEffect(() => {
    async function getUserAgent() {
      try {
        const chValue = await navigator.userAgent();
        const {
          downlink,
          rtt,
          saveData,
  ...
  return <ClientHintsContext.Provider value={ch}>{children}</ClientHintsContext.Provider>;
};
